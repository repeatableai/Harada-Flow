# DCE Meta Prompt — Era 3: Section A19 (Current)

> Commit: `61b154d` — feat: add A19 Functional Render Injection to meta-prompt
> Source: `src/components/flow/DeliverableCreatorStep.jsx:272-828`
> Prompt size: ~560 lines
> Prompts generated: exactly 8 (schema-enforced)
> Added: 3-tier artifact model, functional render injection, engagement modes, refusal envelope.
...
## What was added in Era 3 (vs Era 2)

The entire Section A19 — Functional Render Injection (~200 lines):

- **A19-1** — 3-tier artifact model (Static / Functional In-Chat / Production)
- **A19-2** — Functionality-implying artifact enum (7 strict types)
- **A19-3** — Engagement mode gate (static-only / functional-on-demand / functional-default)
- **A19-4** — Exact scripted prompt text for functional upgrade offer (7th-grade reading level)
- **A19-5** — Functional render behavior (React sandbox, defensive coding, design system, Registry handshake)
- **A19-6** — "What's Next" ACD subsection with production brief template
- **A19-7** — /functional slash command handler
- **A19-8** — Refusal envelope for excluded capabilities (3 pre-written patterns)
- **A19-9** — Size check to prevent truncated renders
- **A19-10** — Synthetic data rules for regulated clients
- **A19-11** — Internal Preview ribbon spec (exact copy + styling)

## What remained unchanged from Era 2

- All 10 DCE Universal Rules
- All 8 prompt definitions (Prompts 1-8)
- Section A governance (font, toggle, WCAG, MCQ, ACD, context window)
- JSON output schema + quality requirement
- Attending Asset Discovery + Parallel Production Queue
