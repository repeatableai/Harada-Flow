# DCE Meta Prompt — Era 2: Section A

> Commit: `9f98a51` — feat(meta-prompt): add A.1-A.5 design rules, MCQ protocol, framework, ACD trigger, context window
> Source: `src/components/flow/DeliverableCreatorStep.jsx:53-404`
> Prompt size: ~350 lines
> Prompts generated: exactly 8 (schema-enforced)
> Full DCE methodology crystallized in a single commit.
...
## What was added in Era 2 (vs Era 1)

- Company URL injection + web context
- "COMPLETE AND SELF-CONTAINED" mandate for each prompt
- 10 DCE Universal Rules (Rules 1-10)
- Fixed 8-prompt structure (up from 3-5)
- Prompt 7 Attending Asset Discovery + Parallel Production Queue
- Prompt 8 Portfolio Hub
- Section A — Always-On Design & Governance Rules
  - Inter font enforcement + exact toggle script
  - WCAG AA contrast
  - MCQ protocol (A-G with complexity signals)
  - MCQ types (Structural/Additive/Stylistic)
  - Framework requirements (5 Whys, Theory of Constraints, 80/20, Kaizen, What/If, Reverse Moonshot)
  - ACD requirement (7 mandatory sections)
  - Context window management with accelerators
  - Handoff document spec
- JSON schema enforcement (`minItems: 8, maxItems: 8`)
- Quality requirement (800-2000+ words per prompt)

## What did NOT exist yet

- Section A19 Functional Render Injection
- 3-tier artifact model (Static / Functional / Production)
- Engagement mode gate
- Functional upgrade prompt
- React sandbox rules
- Refusal envelope
- "What's Next" production brief template
- Internal Preview ribbon spec
